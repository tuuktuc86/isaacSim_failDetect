        ...
        with torch.inference_mode():
            ...
            for env_num in range(num_envs):
               ...
               # RGB 채널만 추출
                ...
                # cv2.imwrite("sim_camera.png", cv2.cvtColor(rgb_np, cv2.COLOR_RGB2BGR))
                #print(camera.get_current_frame())
                # 1. RGBA 데이터 받아오기
                # rgba = camera.get_rgba()  # 1D array of shape (H * W * 4,)
                # print(rgba.shape)
                ...
                # # 4. RGB → BGR (OpenCV는 BGR 형식을 사용함)
                # bgr = cv2.cvtColor((rgb * 255).astype(np.uint8), cv2.COLOR_RGB2BGR)

                # # 5. 저장
                # cv2.imwrite("camera_image.png", bgr)
                # print("Saved camera image to camera_image.png")
                ...
                # image_filename = os.path.join(save_dir, f"rgb_env{env_num}.png")
                # cv2.imwrite(image_filename, img_np)
                # print(f"Saved RGB image to {image_filename}")
