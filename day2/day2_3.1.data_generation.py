...
YCB_OBJECTS_USD_PATH =  os.path.join(os.getcwd(), "data/assets/ycb_usd/ycb/013_apple/final.usd")
...
def get_randomized_scene_cfg() -> tuple[type[InteractiveSceneCfg], list[str]]:
    ...
    num_objects_to_show = random.randint(1)
    ...
    for i in range(num_objects_to_show):
        ...
        pos_x = random.uniform(0.3, 0.6)       # (예제 1) 테이블 위 X 좌표
        pos_y = random.uniform(-0.3, 0.3)      # (예제 2) 테이블 위 Y 좌표
        pos_z = 0.8                            # (예제 3) 공중에서 떨어뜨릴 Z 좌표 (테이블 높이 0.5 + 여유)
        random_scale = random.uniform(0.8, 1.2) # (예제 4) 객체 크기
        random_z_angle = -math.pi + 2 * math.pi * random.random() # (예제 5) Z축 기준 회전 각도 (라디안)
